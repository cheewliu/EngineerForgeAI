...
✅ "Change the entire color scheme from purple (#8b5cf6) to duck yellow
   (#F5C518). Update all CSS variables, hardcoded rgba values, gradients,
...
At one point I asked it to check the whole codebase for old brand names lingering in the code. It found the localStorage key still using the old prefix, tag colours using old brand hex values, and a fallback URL in the RSS feed that nobody would have noticed for months.
...
It remembered the duck. It remembered the brand rules. It remembered which Shiki theme we settled on after changing it three times. If you're building anything non-trivial with Claude Code, a `CLAUDE.md` is not optional.
...
"The about page 'Who's driving' section needs more content.
 ...
 Keep it fun and honest. Change the icon from 🧑‍💻 to 🦆 and add
 a joke that the duck wrote this section."
```

```
"Do a full audit. Check for any remaining old brand names,
 broken internal links, outdated CSS colors, and localStorage
 keys with old prefixes. Report everything by category with
 file paths and line numbers, then fix it all."
```

```
"The code block styling looks too dark and hard to read.
 Change it to look like VS Code — the exact dark-plus theme.
 Background #1e1e1e, border #3c3c3c. Keep the copy button visible."
```
